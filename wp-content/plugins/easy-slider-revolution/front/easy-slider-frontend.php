		...
		if ( ( 'publish' !== $post_status ) || ( 0 === count( $metadata ) ) || ( 'es_slider' !== $post_type ) ) {
			...
			if ( ( 'true' === $slide_data['reverse_order'] ) || ( 'true' === $slide_data['random_order'] ) ) {
				...
				for ( $i = 1; $i <= $slide_data['num_slides']; $i++ ) {
					...
					$slide_data[ 'slide' . $i . '_button_background' ] = $reorder_arr[ $i - 1 ]['button_background'];
					$slide_data[ 'slide' . $i . '_button_color' ]      = $reorder_arr[ $i - 1 ]['button_color'];
			...
			for ( $i = 1; $i <= $slide_data['num_slides']; $i++ ) {
				...
				$output .= "<div class='textContainer'>". $slide_content . "</div><div><a href='" . $slide_data[ 'slide' . $i . '_link_url' ] . "'  type='button'   target='".$slide_data[ 'slide' .$i. '_link_target']."' class='btn SliderButton".$i."'>" . $slide_data[ 'slide' . $i . '_link_text' ]."</a></div></div>\n"; // .sa_hover_container
			}
			$output .= "</div>\n"; // .owl-carousel
